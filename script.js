...
let draggedStickerData = null; // for sticker drag from panel
...
function makeCellDraggable(td) {
  ...
  td.addEventListener('dragover', (e) => {
    ...
    if (draggedStickerData) {
      // Allow sticker drop anywhere inside board area; handled globally
      return;
    }
  ...
  td.addEventListener('drop', (e) => {
    ...
    // If the drag was a sticker from panel, ignore per-cell drop (stickers drop onto board wrapper)
...
resetBtn.addEventListener('click', () => {
  ...
  // Clear all cell contents except time cells
...
/* Sticker dragging from panel to board */
...
/* Accessibility: prevent Enter from inserting divs in contentEditable, keep lines tidy */
document.addEventListener('keydown', (e) => {
  if (e.target && e.target.classList && e.target.classList.contains('cell-content')) {
    if (e.key === 'Enter') {
      e.preventDefault();
      document.execCommand('insertLineBreak'); // single line break
    }
  }
});
